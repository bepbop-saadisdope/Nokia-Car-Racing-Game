...
; Lane positions (3 lanes) - centered in each lane
LANE_LEFT	equ 25
LANE_CENTER	equ 40
LANE_RIGHT	equ 55
...
; Show quit confirmation (software interrupt simulation)
...
draw_box_line_top:
	...
	; Top-left corner
	mov byte [es:di], 0xDA
	mov byte [es:di+1], 0x1F	; White on blue
	add di, 2
...
; Draw middle line (empty or with content)
; Input: BX = row, CX = col
...
; Print string with white on blue background
...
wait_start:
	; Wait for game to start (ISR sets game_started)
	cmp byte [game_started], 0
	je wait_start
